...
habitat.ver<-2 ##version file for the habitat variables
...
pan.world<-function() {
   reg<-locator(1)
   x.range<-(abs(zoom.log[dim(zoom.log)[1],1]- zoom.log[dim(zoom.log)[1],2]))/2
   y.range<-(abs(zoom.log[dim(zoom.log)[1],3]- zoom.log[dim(zoom.log)[1],4]))/2

   zoom.log<<-rbind(zoom.log,c(reg$x[1]-x.range,reg$x[1]+x.range,reg$y[1]-y.range,reg$y[1]+y.range))
   plot.world()
}
...
## define a function to change selected points to a given type
#ideally this should be interactive and update every action we take
change.multiple.vertices<-function(habitat.type=NULL) {
   if (!habitat.type %in% as.character(habitat.details$habitat)) {
    cat("The habitat you chose is not yet described in 'habitat.details'\n")
    return (0)
   }
   if (is.null(habitat.type)) {
    ...
    spoint<-list(x=c(0,0))
    while (length(spoint[[1]])==2) {
      spoint<-NULL
      spoint<-locator(2)
      if (length(spoint[[1]])==2) {
        selected.vertices<-subset(coord.centers,long>min(spoint$x))
        selected.vertices<-subset(selected.vertices,long<max(spoint$x))
        selected.vertices<-subset(selected.vertices,lat>min(spoint$y))
        selected.vertices<-subset(selected.vertices,lat<max(spoint$y))
        ##change habitat type for selected vertices
        vertex.habitat$habitat[as.numeric(row.names(selected.vertices))]<<-habitat.type
        ##replot points
        points(selected.vertices$long,selected.vertices$lat,pch=19,cex=psize.def,col=as.character(habitat.details[habitat.details$habitat==habitat.type,2]))
      }
    }
  }
}
...
##Function to readd special edges
...
##write out the new habitat definition
write.csv(vertex.habitat,paste(file.name,".habitat.v",habitat.ver+1,".csv",sep=""))
...
#################################################
##CODE TO ADD POP LOCATIONS ON THE GRAPH
#################################################
