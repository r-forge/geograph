 ...
 ##first we need to select only points which are not in the current zoomed view
  ...
  if (dim(removed.edges)[1]>0) {
    for (i in 1:dim(removed.edges)[1]) {
      ...
      segments(coord.centers$long[removed.edges[i,1]],coord.centers$lat[removed.edges[i,1]],coord.centers$long[removed.edges[i,2]],coord.centers$lat[removed.edges[i,1]], col="red",lwd=2)
    }
  }
...
remove.edges<-function() {
    ...
    while (length(spoint)>1) {
      ...
      if(length(spoint)>1) {
        ...
        removed.edges[dim(removed.edges)[1]+1,]<<-c(as.numeric(row.names(coord.centers)[spoint[1]]),as.numeric(row.names(coord.centers)[spoint[2]]))
      }
    }
