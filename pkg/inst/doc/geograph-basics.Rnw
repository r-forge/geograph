...
%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%
An overview of the material implemented in the package is summarized in a manpage accessible
by typing:
<<eval=FALSE>>=
?geoGraph
@
...
%%%%%%%%%%%%%%%%%%%%%
Geographic data consist of a set of locations, possibly accompanied with additional information.
For instance, one may want to study the dispersal in a set of biological populations with known
latitude and longitude.
In \code{geoGraph}, geographic data are stored in \code{gData} objects, which matches each
location to a node of a grid (\code{gGraph} object), and stored additional data.
\\


As a toy example, let us consider London (UK), Malaga (Spain), and Zagreb (Croatia).
Since we will be working with a crude grid (10,000 nodes), locations need not be exact.
We enter the longitudes and latitudes (in this order, that is, xy coordinates), in decimal degrees, for these cities:
<<cities>>=
London <- c(0,51)
Malaga <- c(4,37)
Zagreb <- c(16,46)
cities.xy <- rbind(London, Malaga, Zagreb)
colnames(cities.xy) <- c("lon","lat")
cities.xy
@
...
plot(worldgraph.10k)
@
(use \code{worldgraph.40k} for a better resolution).
On this figure, each node is represented with a color depending on the habitat type, either 'sea'
(blue) or 'land' (green).
We will interface the cities data with this grid; to do so, we create a \code{gData} object using
\code{new} (see \code{?gData} object):
<<>>=
cities <- new("gData", coords=cities.xy, gGraph.name="worldgrah.10k")
@
