...
For this, use the interactive functions \code{geo.zoomin}, \code{geo.zoomout}, \code{geo.slide}, and
\code{geo.back}.
All these functions will require you to left-click on the graphics to zoom in, zoom out, or slide to
...
defined by the hidden variable \code{.geoGraphEnv}:
<<>>=
.geoGraphEnv
ls(get(".geoGraphEnv"))
get("last.plot",get(".geoGraphEnv"))
@
...
There are two main ways of editing \code{gGraph} objects, which match two different objectives.
The first approach is to perform global and systematic changes of the connectivity of the graph.
For instance, one may want to remove all connections implying nodes in the sea, if terrestrial
organisms are under study.
