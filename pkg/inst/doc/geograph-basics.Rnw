...
plot(worldgraph.10k)
@
(we could use \code{worldgraph.40k} for a better resolution).
...
<<cities, fig=TRUE>>=
cities <- new("gData", coords=cities.dat[,1:2], data=cities.dat$pop, gGraph.name="worldgraph.10k")
cities
...
Locations can be re-assigned to nodes with restrictions for some node attribute values using \code{closestNode}:
<<closeNode, fig=TRUE>>=
cities <- closestNode(cities, attr.name="habitat", attr.value="land")
plot(cities, type="both")
@

\noindent Now, all cities have been assigned to a node on the grid.
Content of \code{cities} can be accessed via various accessors (see \code{?gData}).
For instance, original locations:
<<>>=
