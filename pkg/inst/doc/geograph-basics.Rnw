...
Note that this object should be subsettable (i.e. the \code{[} operator should be defined), so that
  data can be subsetted when subsetting the \code{gData} object.
\\
