...
% % % % % % % % % % % %
\subsubsection{Changing the connectivity of a \code{gGraph}}
% % % % % % % % % % % %
...
The second approach is to refine the graph by hand, adding or removing locally some connections.
This can be necessary to connect components such as islands to the main landmasses.
Apart from adding or removing connections, other possible operations will concern changing costs
properties as well as changing attribute values.
...
head(colMat)
tail(colMat)
plot(newGraph, col.rules=colMat)
@

As soon as a GIS layer has been extracted to a \code{gGraph}, this information becomes available for
