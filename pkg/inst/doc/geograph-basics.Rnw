...
For this, use the interactive functions \code{geo.zoomin}, \code{geo.zoomout}, \code{geo.slide},
\code{geo.back}, \code{geo.bookmark}, and \code{geo.goto}.
The zoom and slide functions will require you to left-click on the graphics to zoom in, zoom out, or slide to
...
\code{geo.bookmark} and \code{geo.goto} respectively set and go to a bookmark, i.e. a tagged area.
This is most useful when one has to zoom to distant areas repeatedly.
\\
...
R is that the \code{geoGraph} keeps the changes made to the plotting area in memory.
...
@
However, it is recommended not to modify these objects, unless you really know what you are doing.
...
% % % % % % % % % % % %
\code{gData} objects are by default plotted in addition to the corresponding \code{gGraph}.
...
plotted area to the geographic extent of the dataset.
\\
...
\code{gGraph} can be modified in different ways: by changing the connectivity, and by changing costs
...
There are two main ways of chaning the connectivity of a \code{gGraph}, which match two different objectives.
...
Let's assume one is interested in sea fishes.
To model fish dispersal, we have to define a graph which connects only nodes falling in seas.
...
To do so, the easiest approach is to i) define costs for edges based on habitat, with land being
...
edges between nodes.
To do so, we use \code{setCosts}, which sets the cost of an edge to the average of the costs of the nodes:
<<fig=TRUE>>=
...
Note that, despite we restrained the plotting area to Madagascar, this change is effective everywhere.
...
% % % % % % % % % % % %
A second approach to chaning a \code{gGraph} is to refine the graph by hand, adding or removing
locally some connections, or altering the values of node attributes.
This can be necessary to connect components such as islands to the main landmasses, or to correct
erroneous data.
Adding and removing edges from a the grid of a \code{gGraph} can be achieved by \code{geo.add.edges}
...
See \code{?geo.add.edges} for more information on this functions.
...
\noindent Note that when adding nodes is done for an area or for an entire graph, node addition is based on
another \code{gGraph}.
For graph based on 10k or 40k grids, the raw graphs provided in \code{geoGraph} should be used
(\code{rawgraph.10k}, \code{rawgraph.40k}).
\\

We may also want to modify the attributes of specific nodes.
...
(\url{http://...}) format, using the function \code{extractFromLayer}.
...
@
The summary of the object shows the different informations (`\emph{attributes}') stored in the layer.
For now, we assume that we are only interested in retrieving continent and country information for the
\code{worldgraph.10k} object.
Note that \code{extractFromLayer} can handle other types of objects (see \code{?extractFromLayer})
<<>>=
data(worldgraph.10k)
...
We can use the newly acquired information for plotting \code{newGraph}, by defining
<<fig=TRUE>>=
...
%%%%%%%%%%%%%%%%%%%%%
One especially useful application of \code{geoGraph} is the research of least-cost paths between
...
\code{gData}.
Below, we re-use the example of the documentation of these functions, which uses the famous dataset of native
...
pink: coasts).
Genetic theory predicts that genetic diversity within populations should decrease as populations are
...
if not all locations of the \code{gData} are not connected.
This can be done by \code{connectivityPlot}, which has methods for both \code{gGraph} and
...
We have to set the costs of the \code{gGraph}.
...
We shall illustrate first the strictly uniform costs.
After setting a \code{gGraph} with uniform costs, we use \code{dijkstraFrom} to find the shortests
...
@
The object \code{paths} contains the identified paths, as a list with class \code{gPath} (see \code{?gPath}).
...
\noindent On this graph, each path is plotted with a different color, but paths overlap at several places.
We can extract the distances from the `origin' using \code{as.dist.gPath}, and then examine the
relationship between genetic diversity within populations and the distance from the origin:
<<fig=TRUE>>=
...
Alternatively, we could use costs based on habitat.
...
We first define these new costs:
<<>>=
...
Now the new paths are slightly different from the previous ones.
...
Of course, the distinction between coasts and inner landmasses is a somewhat poor description of the habitat.
