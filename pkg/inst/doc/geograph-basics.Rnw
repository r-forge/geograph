...
As a toy example, let us consider three locations: London (UK), Malaga (Spain), and Zagreb (Croatia).
Since we will be working with a crude grid (10,000 nodes), locations need not be exact.
We enter the longitudes and latitudes (in this order, that is, xy coordinates) of these cities in decimal degrees:
<<cities>>=
London <- c(0,51)
Malaga <- c(4,37)
Zagreb <- c(16,46)
